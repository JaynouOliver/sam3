"""
Full pipeline: SAM3 label 1000 room images -> prep YOLO dataset -> train YOLOv8n.
Optimized with I/O overlap for maximum throughput.
"""
...
LABEL_DIR = "/teamspace/studios/this_studio/sam3_room_labels"
DATASET_DIR = "/teamspace/studios/this_studio/yolo_room_dataset"
TRAIN_DIR = "/teamspace/studios/this_studio/yolo_room_training"
MIN_CONFIDENCE = 0.65
MIN_AREA = 400
SPLIT_RATIO = 0.85
SEED = 42
...
# ═══════════════════════════════════════════════════════════════════
print("=" * 70)
print("STEP 1: SAM3 Auto-Labeling")
print("=" * 70)
...
total_raw = 0
total_filtered = 0
valid_pairs = []  # (image_path, label_path) for dataset prep
...
while True:
    ...
    try:
        ...
        if processed % 50 == 0 or processed == len(image_paths):
            ...
            print(f"  [{processed}/{len(image_paths)}] rate={rate:.2f} img/s | "
                  f"raw={total_raw} filt={total_filtered} valid={len(valid_pairs)} | ETA={eta:.0f}s")
...
print(f"Valid images (with detections): {len(valid_pairs)}")
print(f"Raw: {total_raw} | Filtered: {total_filtered}")
...
print(f"Training: {total_time - label_time:.1f}s")
print(f"Best model: {TRAIN_DIR}/v2_room_distilled/weights/best.pt")
